    ...
    pub const pkgpb = struct {
        pub const Packet = struct {
            ...
            pub fn deinit(self: *const Packet, allocator: all.Allocator) void {
                std.debug.print("Packet.deinit messages={d}\n", .{self.messages.len});
            ...
            fn deseriigi(allocator: all.Allocator, buffer: *DecodeBuffer, data_length: ?usize) !Packet {
                ...
                while (buffer.read_index < end) {
                    ...
                    if (field_number == 1 and wire_type == 2) {
                        ...
                        // try messages_list.append(allocator, try Msg.k6bus.msg.Msg.deseriigiElBin(allocator, try buffer.decodeBytes(try buffer.decodeVarint()), .BF_PROTOBUF));
                        const raw =
                            try buffer.decodeBytes(try buffer.decodeVarint());
                        defer allocator.free(raw);

                        const msg =
                            try Msg.k6bus.msg.Msg.deseriigiElBin(allocator, raw, .BF_PROTOBUF);
