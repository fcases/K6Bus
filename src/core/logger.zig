...
pub const Level = enum(u8) { err=0, warning=1, info=2, trace=3};
...
pub const Logger = struct {
    ...
    pub fn init(allocator: std.mem.Allocator, active: bool, level_int: i32) !Logger { 
        var l= Logger{
            .allocator=allocator,
            .active=active,
            .level= switch(level_int) {
                0 => .err,
                1 => .warning,
                2 => .info,
                else => .trace
        ...
        if(active) { 
            l.file_name=try std.fmt.allocPrint(allocator,"{d}.log",.{std.time.timestamp()}); 
    ...
    pub fn write(self:*Logger,lvl:Level, comptime fmt:[]const u8, args:anytype, src:std.builtin.SourceLocation) void { 
        ...
        var wr=f.writer(); 
        wr.print("{s}:\t{d}\n\t{s}, L-{d}: {s}\n\t",.{lvl.label(),std.time.timestamp(), std.fs.path.basename(src.file), src.line,src.fn_name}) catch return; 
        wr.print(fmt,args) catch return; 
        wr.print("\n",.{}) catch return; 
