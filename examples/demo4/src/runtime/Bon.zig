...
pub const Cia = struct {
    ...
    pub fn initDefault(allocator: all.Allocator) !Cia {
    ...
    pub fn deinit(self: *const Cia, allocator: all.Allocator) void {
        allocator.free(self.name);
        allocator.free(self.ubicacion);
    }

    pub fn plenigiDefaultojn(self: *Cia, allocator: all.Allocator) !void {
        _ = self;
        _ = allocator;
    }

    pub fn skribiAlTeksto(self: *Cia, allocator: all.Allocator, t_formato: TekstaFormato) ![]const u8 {
        return try skribiTiponAlTeksto(allocator, Cia, @as(*Cia, self), t_formato);
    }

    pub fn skribiAlDosiero(self: *Cia, allocator: all.Allocator, path: []const u8, t_formato: TekstaFormato) !void {
        try skribiTiponAlDosiero(allocator, Cia, @as(*Cia, self), path, t_formato);
    }

    pub fn legiElTeksto(allocator: all.Allocator, input: []const u8, t_formato: TekstaFormato) !Cia {
        return try legiTiponElTeksto(allocator, Cia, input, t_formato);
    }

    pub fn legiElDosiero(allocator: all.Allocator, path: []const u8, t_formato: TekstaFormato) !Cia {
        return try legiTiponElDosiero(allocator, Cia, path, t_formato);
    }

    fn skribiAlProtobufTeksto(self: *const Cia, allocator: all.Allocator,ind: []const u8) ![]const u8 {
        ...
        try bufro.print(allocator,"{s}pos_x: {any}\n",.{ind, self.pos_x });
        try bufro.print(allocator,"{s}pos_y: {any}\n",.{ind, self.pos_y });

        return bufro.toOwnedSlice(allocator);
    }

    fn legiElProtobufTeksto(allocator: all.Allocator, it: *TokenIterType) !Cia {
        var mia_Mesagho = try Cia.initDefault(allocator);
        ...
        while (it.next()) |tok| {
            ...
            if( equal(u8, tok, "pos_x" ) ) {
                mia_Mesagho.pos_x =  try std.fmt.parseFloat(f32,val);
                continue;
            }
            if( equal(u8, tok, "pos_y" ) ) {
                mia_Mesagho.pos_y =  try std.fmt.parseFloat(f32,val);
                continue;
            }
        }
    ...
    pub fn seriigiAlBin(self: *const Cia, allocator: all.Allocator, b_formato: BinaraFormato) ![]const u8 {
        return try seriigiTiponAlBin(allocator, Cia, self, b_formato);
    }

    pub fn seriigiAlDosiero(self: *const Cia, allocator: all.Allocator, path: []const u8, b_formato: BinaraFormato) !void {
        return try seriigiTiponAlDosiero(allocator, Cia, self, b_formato, path);
    }

    fn seriigi(self: *const Cia, allocator: all.Allocator, buffer: *EncodeBuffer) !usize {
 
        _ = allocator;
        var tuta_longo: usize = 0;
 
        tuta_longo += try buffer.encodeFloat( self.pos_y );
        tuta_longo += try buffer.encodeVarint(37);
        //5 req - no def - no varlong

        tuta_longo += try buffer.encodeFloat( self.pos_x );
    ...
    pub fn deseriigiElBin(allocator: all.Allocator,input: []const u8, b_formato: BinaraFormato) !Cia {
        return try deseriigiTiponElBin(allocator, Cia, input, b_formato);
    }

    pub fn deseriigiElDosiero(allocator: all.Allocator, path: [:0]const u8, b_formato: BinaraFormato) !Cia {
        return try deseriigiTiponElDosiero(allocator, Cia, path, b_formato);
    }

    fn deseriigi(allocator: all.Allocator, buffer: *DecodeBuffer, data_length: ?usize) !Cia {
        var mia_Mesagho = try Cia.initDefault(allocator);
        errdefer mia_Mesagho.deinit(allocator);
        ...
        while (buffer.read_index < end) {
            ...
            else if ( field_number == 2 and wire_type == 2 ) 
            {
                ...
                mia_Mesagho.pos_x = try buffer.decodeFloat()
            else if ( field_number == 4 and wire_type == 5 ) 
                mia_Mesagho.pos_y = try buffer.decodeFloat();
        }
...
pub const Bon = struct {
    ...
    pub fn initDefault(allocator: all.Allocator) !Bon {
        const mia_name = try allocator.dupe(u8, "");
        errdefer allocator.free(mia_name);
        const mia_ubicacion = try allocator.dupe(u8, "");
        errdefer allocator.free(mia_ubicacion);
        const mia_cia = try allocator.alloc(Cia, 0);
        errdefer allocator.free(mia_cia);
        return Bon {
            .name = mia_name,
            .ubicacion = mia_ubicacion,
            .pos_x = 0,
    ...
    pub fn deinit(self: *const Bon, allocator: all.Allocator) void {
        allocator.free(self.name);
        allocator.free(self.ubicacion);
        for (self.cia) |item| {
            item.deinit(allocator);
        }
        if (self.cia.len > 0) allocator.free(self.cia);
    }

    pub fn plenigiDefaultojn(self: *Bon, allocator: all.Allocator) !void {
        for (self.cia) |*v| try v.plenigiDefaultojn(allocator);
    }

    pub fn skribiAlTeksto(self: *Bon, allocator: all.Allocator, t_formato: TekstaFormato) ![]const u8 {
        return try skribiTiponAlTeksto(allocator, Bon, @as(*Bon, self), t_formato);
    }

    pub fn skribiAlDosiero(self: *Bon, allocator: all.Allocator, path: []const u8, t_formato: TekstaFormato) !void {
        try skribiTiponAlDosiero(allocator, Bon, @as(*Bon, self), path, t_formato);
    }

    pub fn legiElTeksto(allocator: all.Allocator, input: []const u8, t_formato: TekstaFormato) !Bon {
        return try legiTiponElTeksto(allocator, Bon, input, t_formato);
    }

    pub fn legiElDosiero(allocator: all.Allocator, path: []const u8, t_formato: TekstaFormato) !Bon {
        return try legiTiponElDosiero(allocator, Bon, path, t_formato);
    }

    fn skribiAlProtobufTeksto(self: *const Bon, allocator: all.Allocator,ind: []const u8) ![]const u8 {
        var bufro:std.ArrayList(u8)= .empty;
        ...
        for(self.cia) |obj| {
            ...
            const cia_text = try obj.skribiAlProtobufTeksto(allocator, indent);
            defer allocator.free(cia_text);

            try bufro.print(allocator, "{s}cia {{\n{s}{s}}}\n", .{ ind, cia_text, ind });
        }

        return bufro.toOwnedSlice(allocator);
    }

    fn legiElProtobufTeksto(allocator: all.Allocator, it: *TokenIterType) !Bon {
        var mia_Mesagho = try Bon.initDefault(allocator);
        errdefer mia_Mesagho.deinit(allocator);

        var cia_list: std.ArrayList(Cia) = .empty;
        errdefer {
            for (cia_list.items) |*item| {
                item.deinit(allocator);
            }
            cia_list.deinit(allocator);
        }
        ...
        while (it.next()) |tok| {
            ...
            if( equal(u8, tok, "cia" ) ) {
                ...
                const sub_msg = try Cia.legiElProtobufTeksto(allocator, it); 
                cia_list.append(allocator, sub_msg) catch |err| {
        ...
        for (mia_Mesagho.cia) |item| {
            item.deinit(allocator);
        }
        allocator.free(mia_Mesagho.cia);
        mia_Mesagho.cia = try cia_list.toOwnedSlice(allocator); 

        return mia_Mesagho;
    }

    pub fn seriigiAlBin(self: *const Bon, allocator: all.Allocator, b_formato: BinaraFormato) ![]const u8 {
        return try seriigiTiponAlBin(allocator, Bon, self, b_formato);
    }

    pub fn seriigiAlDosiero(self: *const Bon, allocator: all.Allocator, path: []const u8, b_formato: BinaraFormato) !void {
        return try seriigiTiponAlDosiero(allocator, Bon, self, b_formato, path);
    }

    fn seriigi(self: *const Bon, allocator: all.Allocator, buffer: *EncodeBuffer) !usize {
 
        var tuta_longo: usize = 0;
 
        var cia_i: usize = self.cia.len;
        while (cia_i > 0) {
            cia_i -= 1;
            const item = self.cia[cia_i];
            const cia_longa = try item.seriigi( allocator, buffer );
            tuta_longo += cia_longa;
            tuta_longo += try buffer.encodeVarint(cia_longa);
    ...
    pub fn deseriigiElBin(allocator: all.Allocator,input: []const u8, b_formato: BinaraFormato) !Bon {
        return try deseriigiTiponElBin(allocator, Bon, input, b_formato);
    }

    pub fn deseriigiElDosiero(allocator: all.Allocator, path: [:0]const u8, b_formato: BinaraFormato) !Bon {
        return try deseriigiTiponElDosiero(allocator, Bon, path, b_formato);
    }

    fn deseriigi(allocator: all.Allocator, buffer: *DecodeBuffer, data_length: ?usize) !Bon {
        var mia_Mesagho = try Bon.initDefault(allocator);
        errdefer mia_Mesagho.deinit(allocator);
        ...
        var cia_list: std.ArrayList(Cia) = .empty; 
        errdefer {
            for (cia_list.items) |*it| it.deinit(allocator);
            cia_list.deinit(allocator);
        }
        ...
        while (buffer.read_index < end) {
            ...
            { 
                try cia_list.append( 
                    allocator, 
                    try Cia.deseriigi(allocator, buffer, try buffer.decodeVarint() )
                );
            }
        }

        const tmp_cia = try cia_list.toOwnedSlice(allocator);
        for (mia_Mesagho.cia) |*item| {
            item.deinit(allocator);
        }
        allocator.free(mia_Mesagho.cia);
        mia_Mesagho.cia = tmp_cia;
